...
const LOGO_SVG = (
  <svg xmlns="http://www.w3.org/2000/svg" xmlSpace="preserve" height="36"
    viewBox="0 0 10164.01 2350.66">
    <style>{`.lf0{fill:#A53692}.lf1{fill:#ffffff}.lf2{fill:#00AFEF}.lf4{fill:#ffffff}`}</style>
...
export default function LoginPage({ onLogin }: Props) {
  const [email, setEmail] = useState('admin@mohr.com');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setErro(data?.error?.message || 'Email ou senha inválidos');
        return;
      }
          ...
          <form onSubmit={handleSubmit}>
            <div className="field">
              ...
              <input
                ...
                placeholder="admin@mohr.com" autoFocus required
              />
            </div>
            <div className="field">
              <label>Senha</label>
            ...
            {erro && (
              <div style={{
                ...
                fontSize: '13px', color: '#ef4444', fontFamily: 'var(--mono)',
              }}>
                {erro}
              </div>
            )}
